#define FAKE_2FA_FOR_TESTING FALSE

// This is in its own file as it has so much stuff to contend with
/client/proc/edit_2fa()
	if(IsAdminAdvancedProcCall())
		to_chat(usr, "<span class='boldannounce'>2fa change blocked: Advanced ProcCall detected.</span>")
		message_admins("[key_name(usr)] attempted to edit 2fa settings via advanced proc-call")
		log_admin("[key_name(usr)] attempted to edit 2fa settings via advanced proc-call")
		return

	if(!GLOB.configuration.system.api_host && !FAKE_2FA_FOR_TESTING)
	...
	switch(option)
		...
		if("Deactivate")
			...
			if(!entered_code)
				...
				return
			if(!validate_2fa_code(entered_code))
				return
...
/client/proc/validate_2fa_code(entered_code)
	if(FAKE_2FA_FOR_TESTING)
		return TRUE
	...
	if(vr.status_code != 200)
		// See if its unauthorised. I used 400 for that dont at me
		if(vr.status_code == 400)
			alert(usr, "Invalid code entered.")
		else
