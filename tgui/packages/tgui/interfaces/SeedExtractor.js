...
export const SeedExtractor = (properties, context) => {
  ...
  return (
    <Window theme="hydroponics" width={800} height={900}>
      <ComplexModal />
      <Window.Content>
        <Stack fill vertical>
          <SeedList />;
...
const seedFilter = (searchText) => {
  ...
  for (let term in terms) {
    ...
    switch (parts[0].toLocalLowerCase()) {
      case 'l':
      case 'life':
      case 'lifespan':
        ...
        <Section fill scrollable>
          <Table className="SeedExtractor__list">
            <Table.Row bold>
              ...
              <SortButton id="stock">Stock</SortButton>
            </Table.Row>
            {seeds
              ...
              .map((seed) => (
                <Table.Row
                  key={seed.id}
                  className={'SeedExtractor__listRow'}
                  onClick={() =>
                    act('vend', {
                      seed_id: seed.id,
  ...
  return (
    <Stack fill>
      ...
      <Stack.Item>
        ...
        <Input
          placeholder="1"
          onInput={(e, value) => setVendAmount(Number(value))}
        />
      </Stack.Item>
    </Stack>
  );
