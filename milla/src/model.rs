...
pub(crate) struct Tile {
    ...
    /// How much the pressure differential is pulling this tile's air towards +axis.
    pub(crate) stress: [f32; AXES.len()],
    /// How strongly the air in this tile is flowing towards +axis.
    pub(crate) momentum: [f32; AXES.len()],
...
impl Tile {
    ...
    pub(crate) fn copy_from(&mut self, other: &Tile) {
        ...
        for i in 0..AXES.len() {
            self.stress[i] = other.stress[i];
            self.momentum[i] = other.momentum[i];
