...
/// The two axes, Y and X. The order is arbitrary, but will break things if changed.
...
pub(crate) const MINIMUM_NONZERO_MOLES: f32 = 0.01;

/// Balancing factor to adjust the spring force applied by momentum.
pub(crate) const MOMENTUM_MULTIPLIER: f32 = 1.0;

/// Balancing factor to adjust the rate at which momentum decays over time.
/// Multiplier on previous tick's momentum, so 1.0 would be no decay, and 0.0 would prevent all
/// carry-over.
pub(crate) const MOMENTUM_DECAY: f32 = 0.5;

/// Balancing factor to adjust the strenght of wind generated by air momentum.
